    ...
    private val receiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            ...
            if (downloadID == id) {
                ...
                checkDownloadStatus()
                notificationManager.sendCompleteDownloadFile(
                    downloadFileName,
                    downloadStatus,
                    this@MainActivity
                )
        ...
        if (cursor.moveToNext()) {
            ...
            when (status) {
                DownloadManager.STATUS_FAILED -> {
                    ...
                    Timber.d("Download Status ->  | STATUS_FAIL")
                    downloadStatus = "Failed"
                    // do something when failed
                }
                DownloadManager.STATUS_PENDING, DownloadManager.STATUS_PAUSED -> {
                    // do something pending or paused
                    Timber.d("Download Status -> STATUS_PAUSED | STATUS_PENDING")
                }
                DownloadManager.STATUS_SUCCESSFUL -> {
                    binding.lyContent.btnDownload.updateButtonState(ButtonState.Completed)
                    downloadStatus = "Success"
                    Timber.d("Download Status -> STATUS_SUCCESSFUL")
                    // do something when successful
                }
                DownloadManager.STATUS_RUNNING -> {
                    // do something when running
                    Timber.d("Download Status -> STATUS_RUNNING")
                }
            }
        }
    }
